...
/* Local */
class PatternsWoocommerceSettingsApi {
    constructor() {
        this.endpoint = 'patterns-woocommerce/v1/settings';
    ...
    bindEvents() {
        document.addEventListener('click', (event) => {
            if (event.target.classList.contains('patterns-woocommerce-gsn-close')) {
                this.modifySettings('hide_get_started_notice', true);
            }
            else if (event.target.classList.contains('patterns-woocommerce-rn-permanent-close')) {
                this.modifySettings('remove_review_notice_permanently', true);
            }
            else if (event.target.classList.contains('patterns-woocommerce-rn-temporary-close')) {
                this.modifySettings('remove_review_notice_temporary_date_time', 1);
            }
            else if (event.target.classList.contains('patterns-woocommerce-install-plugins')) {
    ...
    updateUI() {
        if (this.settings.hide_get_started_notice) {
            const noticeElement = document.getElementById('patterns-woocommerce-gsn');
            if (noticeElement) {
        ...
        if (!this.isLoadReview()) {
            const reviewNoticeElement = document.getElementById('patterns-woocommerce-review-notice');
    ...
    async installPlugins() {
        const plugins = PatternsWoocommerceLocalize.recommended_plugins;
        ...
        window.location.replace(PatternsWoocommerceLocalize.theme_info_url);
    }
}

document.addEventListener('DOMContentLoaded', () => {
    new PatternsWoocommerceSettingsApi();
});
